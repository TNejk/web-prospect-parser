...
class ProspectParser:
    def __init__(self, category: str = '/hypermarkte/'):
    ...
    def _fetch_validate_store_prospects(self) -> None:
        for link, store in self.store_links:
            ...
            for prospect in prospects.find_all(class_='brochure-thumb', name='div'):
                ...
                for data in prospect_data:
                    if data.name == 'h2':
                        ...
                        dates: list[str] = self._reformat_dates(data.get_text().strip())
                        
                        if self._is_valid_prospect(dates):
                            temp_storage['valid_from'] = dates[0].strip()
                            if len(dates) == 2:
                                temp_storage['valid_to'] = dates[1].strip()
                            else:
                ...
                if len(temp_storage) == 5:
                    temp_storage['parsed_time'] = datetime.today().strftime('%Y-%m-%d %H:%M:%S').strip()
